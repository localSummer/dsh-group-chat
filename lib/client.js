	...
	factory: (require) => {
		...
		*/
		...
		function Composer(props) {
			const { snap, sess, group, enabledRoles, participants, mentionedRoles, busyNow, input, mention, mentionCandidates, mentionIdxC, rounds, err, atBottom, bubblesLength, inputRef, scrollRef, setMentionIdx, setRounds, togglePart, onInputCE, onInputKeyDown, onPasteCE, onDropCE, onDragOverCE, insertChip, sendMsg, stopRun, mutate, setMention } = props;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		function ChatPanel(props) {
			const { snap, sess, group, enabledRoles, participants, mentionedRoles, busyNow, input, mention, mentionCandidates, mentionIdxC, rounds, err, atBottom, topicDraft, msgById, navOpen, asideOpen, inputRef, scrollRef, setTopicDraft, setConfirmClear, setMentionIdx, setRounds, setNavOpen, setAsideOpen, togglePart, onMsgsScroll, onInputCE, onInputKeyDown, onPasteCE, onDropCE, onDragOverCE, insertChip, sendMsg, stopRun, action, mutate, setMention } = props;
			const commitTopic = () => {
		...
		*/
		...
		function useMentionChip(inputRef, setMention, setMentionIdx, syncFromDOM) {
			return { insertChip: (0, react.useCallback)((role) => {
				const el = inputRef.current;
				const sel = window.getSelection();
				if (!el || !sel) return;
		...
		function useInputKeyboard(mention, mentionCandidates, mentionIdxC, setMentionIdx, insertChip, sendMsg) {
		...
		*/
		function GroupChatPanel() {
			...
			if (sess && group && snap && sess.groupId !== group.id) sess = group.sessionIds.length ? sessById(snap, group.sessionIds[group.sessionIds.length - 1]) : null;
