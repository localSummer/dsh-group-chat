...
function sanitizeReactions(value) {
	...
	for (const e of value) if (typeof e === "string" && REACTION_EMOJIS.includes(e)) set.add(e);
	if (!set.size) return void 0;
...
function createActions(core, deps) {
	...
	const mutate = (args) => {
		...
		if (op === "createGroup") {
			...
			msg.reactions = cur.includes(emoji) ? cur.length > 1 ? cur.filter((e) => e !== emoji) : void 0 : REACTION_EMOJIS.filter((e) => e === emoji || cur.includes(e));
...
function createBroadcast(core) {
	...
	const snapshot = () => ({
		...
		run: {
			...
			queue: core.run.queue.slice(),
			queueIndex: core.run.queueIndex,
			turnStartedAt: core.run.turnStartedAt,
...
function createConversation(core, deps) {
	...
	const speakDuration = () => Math.max(0, Date.now() - (run.turnStartedAt ?? Date.now()));
		...
		if (doc) {
			...
			for (const m of Array.isArray(doc.messages) ? doc.messages : []) {
				...
				core.messages.set(m.id, {
					...
					durationMs: typeof m.durationMs === "number" && m.durationMs > 0 ? m.durationMs : void 0,
					ts: typeof m.ts === "number" ? m.ts : Date.now()
				});
